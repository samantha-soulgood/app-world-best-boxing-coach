...
const App: React.FC = () => {
  ...
  const generateWorkoutPlan = useCallback(async (options: {
    ...
    prompt += '2. **Main Workout Circuit**: 4 exercises (45s-1min each) + Rest (15s) after each. Add "Repeat this circuit X times" in last exercise notes (X=2 for 20min, X=3 for 30+min)\n';
    ...
    const requestBody = {
      ...
      messages: [
        {
          role: "system",
          content: `${SAMMI_PERSONA}\n\nRespond with valid JSON matching this schema: ${JSON.stringify(workoutSchema)}\n\nMain Workout = circuit format: 4 exercises with Rest (15s) between. Last exercise notes must say "Repeat this circuit X times".`
        },
        {
          role: "user", 
          content: prompt

...
const Journal: React.FC<JournalProps> = ({ entries, onClose, onAddEntry }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in">
      <div className="w-full max-w-lg bg-white border border-gray-300 rounded-lg flex flex-col max-h-[90vh]">
        ...
        <div className="overflow-y-auto p-4 flex-1 bg-gray-50">
          {entries.length === 0 ? (
            ...
            <ul className="space-y-3">
              {entries.map((entry) => (
                <li key={entry.id} className="bg-white border border-orange-200 p-4 rounded-lg">
        ...
        <footer className="p-4 border-t-2 border-orange-200 flex-shrink-0 bg-white">
          <textarea
            ...
            className="w-full bg-white border border-orange-200 rounded-lg p-3 text-gray-800 placeholder:text-gray-500 focus:ring-2 focus:ring-orange-400 focus:border-orange-400 focus:outline-none resize-none transition-all duration-200"
            rows={3}
          />
          <div className="flex justify-between items-center mt-3">
            ...
            <button
                ...
                className="text-sm font-bold text-gray-900 bg-gradient-to-r from-orange-100 to-amber-100 hover:from-orange-200 hover:to-amber-200 rounded-lg px-4 py-2.5 disabled:bg-gray-300 disabled:cursor-not-allowed transition-all duration-200 border border-orange-300 flex items-center gap-2"
            >
                <span>💾</span>

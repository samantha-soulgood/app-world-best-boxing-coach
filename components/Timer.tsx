...
const Timer: React.FC<TimerProps> = ({ initialSeconds, onComplete, autoStart = false, isPaused = false }) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center">
      {/* Single progress circle - no overlapping elements */}
      <svg className="w-44 h-44 sm:w-52 sm:h-52 transform -rotate-90 mb-6">
        <defs>
        ...
        {/* Time display centered in the circle */}
        <text 
          x="50%" 
          y="50%" 
          textAnchor="middle" 
          dominantBaseline="middle"
          className={`text-[2.2rem] sm:text-[2.8rem] font-mono font-bold transition-all duration-500 ${
            isVeryLowTime ? 'fill-red-400' : 
            isLowTime ? 'fill-orange-400' : 'fill-white'
          }`}
      ...
      {/* Control button below the circle */}
      <button 
        onClick={toggle} 
